...
export class ResourcePoolManager {
    private pools = new Map<string, { limit: number; active: number; queue: QueuedRequest[]; totalAcquired: number; totalWaitTimeMs: number }>();
    ...
    async acquire(
        ...
        if (!pool) {
            ...
            pool = { limit: this.globalLimit, active: 0, queue: [], totalAcquired: 0, totalWaitTimeMs: 0 };
        ...
        return new Promise<ReleaseFunction>((resolve, reject) => {
            ...
            if (options.signal) {
                options.signal.addEventListener('abort', () => {
                    const index = pool!.queue.indexOf(request);
                    if (index !== -1) {
